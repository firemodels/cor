...
For a steady-state fire, the correlation of Alpert~\cite{SFPE:Alpert} predicts that the activation time of a sprinkler, $t_{\rm act}$~(\si{s}), is given by~\cite{NFPA}
\be
t_{\rm act} =  \frac{\mathrm{RTI}}{\sqrt{u_{\rm jet}}} \ln \left( \frac{T_{\rm jet} - T_\infty}{T_{\rm jet} - T_{\rm act}} \right)
\label{eq:Alpert}
\ee
where RTI is the response time index of the sprinkler~(\si{(m.s)^{1/2}}), $T_\infty$ is the ambient air temperature~(\si{\celsius}), and $T_{\rm act}$ is the activation temperature of the sprinkler~(\si{\celsius}). The ceiling jet temperature, $T_{\rm jet}$ (\si{\celsius}), is given by
\be
T_{\rm jet} = \left\{ \begin{array}{cl}
...
The ceiling jet velocity, $u_{\rm jet}$ (\si{m/s}), is given by
\be
u_{\rm jet} = \left\{ \begin{array}{cl}
...
\multicolumn{2}{|l|}{$T_{\rm act}$ (\si{\celsius})}  &  \multicolumn{1}{c|}{68}          \\ \hline
