...
\be
A_{\rm s} \frac{dz}{dt} = \frac{dV_{\rm ul}}{dt} = \dot V_{\rm ul}
\label{eq:ASET_1}
\ee
where $A_{\rm s}$ is the area of the boundary surfaces~(\si{m^2}), and $V_{\rm ul}$ is the volume of the HGL~(\si{m^3}).
The change in volume of the upper layer, $\dot V_{\rm ul}$~(\si{m^3/s}), is given by
\be
\dot V_{\rm ul} = \dot V_{\rm exp} + \dot V_{\rm ent}
\label{eq:ASET_2}
\ee
The volumetric expansion rate, $\dot V_{\rm exp}$~(\si{m^3/s}), is given by~\cite{SFPE:Mowrer}
\be
\dot V_{\rm exp} = \frac{\dot Q_{\rm net}}{\rho_{\rm g} c_{\rm p} T_{\rm g}} \approx \frac{(1 - \chi_{\rm l}) \dot Q_{\rm f}}{353} \quad ; \quad \dot Q_{\rm net} = \dot Q_{\rm f} - \dot Q_{\rm l} = \dot Q_{\rm f}  (1 - \dot \chi_{\rm l})
\label{eq:ASET_3}
\ee
where $\dot Q_{\rm net}$, $\dot Q_{\rm f}$, and $\dot Q_{\rm l}$ are the net HRR, total HRR, and HRR loss to the boundaries~(\si{kW}), respectively, $\rho_{\rm g}$, $c_{\rm p}$ and $T_{\rm g}$ are the density~(\si{kg/m^3}), specific heat~(\si{kJ/(kg.K)}), and temperature~(\si{K}) of air in the HGL, respectively, and $\chi_{\rm l}$ is the heat loss fraction to the enclosure boundaries.
The volumetric entrainment rate, $\dot V_{\rm ent}$~(\si{m^3/s}), is given by~\cite{Zukoski:1981}
\be
\dot V_{\rm ent} = k_{\rm v} \dot Q^{1/3} z^{5/3} = \frac{0.21}{K_{\rm f}} \left( \frac{g}{\rho_\infty T_\infty} \right)^{1/3} (K_{\rm f} \dot Q)^{1/3} (z - z_{\rm f})^{5/3}
\label{eq:ASET_4}
\ee
where $k_{\rm v}$ is the volumetric entrainment coefficient, $g$ is the acceleration due to gravity~(\si{m/s^2}), $\rho_\infty$ and $T_\infty$ are the density~(\si{kg/m^3}) and temperature~(\si{K}) of ambient air, respectively, $K_{\rm f}$ is the location factor, and $z_{\rm f}$ is the fuel height~(\si{m}). The location factor has a value of 1, 2, or 4, which corresponds to a fire away from walls or corners, a fire adjacent to a wall, or a fire located in a corner, respectively.

The HGL height, $z$, in Eq.~\ref{eq:ASET_1} can be calculated iteratively using
\be
z|_{t+1} = z|_t - \frac{\dot V_{\rm ul}}{L W} \Delta t
\label{eq:ASET_5}
\ee
...
A summary of the comparisons between peak predicted and measured HGL depths is shown in Fig.~\ref{HGL Depth (ASET)}.
...
\be
z = \left( \frac{2 k \dot Q^{1/3} t}{3 A_{\rm c}} + \frac{1}{h_{\rm c}^{2/3}} \right)^{-3/2}
\label{eq:Yamana_Tanaka}
\ee
where $\dot Q$ is the HRR~(\si{kW}), $t$ is the time after ignition~(\si{s}), $A_{\rm c}$ is the compartment floor area~(\si{m^2}), and $h_{\rm c}$ is the compartment height~(\si{m}). The constant $k$ is given by
\be
k = \frac{0.076}{(353/T_{\rm g})}
\ee
where $T_{\rm g}$ is the HGL temperature~(\si{K}).
...
Note: In this verification case, the Beyler method (see Section~\ref{sec:Beyler}) is used to calculate the HGL temperature, $T_{\rm g}$.
...
A summary of the comparisons between peak predicted and measured HGL depths is shown in Fig.~\ref{HGL Depth (Yamana and Tanaka)}.
...
\label{HGL Depth (Yamana and Tanaka)}
\end{figure}
