...
The radiative heat flux, $\dot q''_{\rm r}$~(\si{kW/m^2}), at any distance $R$ (\si{m}) from this point can be predicted using
\be
\dot q''_{\rm r} = \cos\theta \left( \frac{\chi_{\rm r} \dot Q}{4 \pi R^2} \right)
\label{eq:point_source}
\ee
where the $\cos\theta$ term (equal to $x/R$ for targets facing sideways, or $z/R$ for gauges facing upward or downward) accounts for a target that is at an angle $\theta$ from the source. In the Fortran implementation of this empirical correlation, the IOR orientation parameter is used to specify which direction the target or gauge is facing: 1 or -1 for the positive or negative $x$ direction, 2 or -2 for the positive or negative $y$ direction, and 3 or -3 for the positive or negative $z$ direction. In Eq.~\ref{eq:point_source}, $\chi_{\rm r}$ is the radiative fraction (unless provided in the test report, a value of 0.35 was used), $\dot Q$ is the HRR of the fire~(\si{kW}), and $R$ is the radial distance from the point source to the edge of the target~(\si{m}) and is given by
\be
R = \sqrt{x^2 + \left(z - \frac{L_{\rm f}}{3} \right)^2}
\label{eq:point_source_R}
\ee
where $x$ is the horizontal distance from the point source to the edge of the target~(\si{m}), and $z$ is the height of the heat flux target~(\si{m}). The flame height, $L_{\rm f}$ (\si{m}), is given by
\be
L_{\rm f} =0.235 Q^{2/5} - 1.02 D
...
\be
Q^* = \frac{\dot Q}{\rho_\infty c_{\rm p} T_\infty \sqrt{g} D^{5/2}}
\label{eq:point_source_Qstar}
\ee
where $\rho_\infty$ is the ambient air density~(\si{kg/m^3}), $c_{\rm p}$ is the specific heat of air~(\si{kJ/(kg.K)}), $T_\infty$ is the ambient air temperature~(\si{K}), and $g$ is the acceleration of gravity~(\si{m/s^2}).
...
% In the FDTs spreadsheets, the point source radiation heat flux is calculated without accounting for the angle/elevation of the target with respect to the fire, and the flame height is not considered. The correlations.f90 program considers both of these and assumes that the point source is located one-third of the height of the flame ($L_{\rm f}/3$).
...
% \noindent Expected result (FDTs spreadsheets): At a radius $R$ of 0.67~m, the radiative heat flux $\dot q''_{\rm r}$ is 6.22~kW/m$^2$; at an $R$ of 0.92~m, $\dot q''_{\rm r}$ is 3.30~kW/m$^2$; at an $R$ of 1.17~m, $\dot q''_{\rm r}$ is 2.04~kW/m$^2$.
...
The radiative heat flux, $\dot q''_{\rm r}$~(\si{kW/m^2}), at any distance $R$ (\si{m}) from the center of the flame can be predicted using
\be
\dot q''_{\rm r} = E F_{12}
...
\be
\frac{\chi_{\rm r} \dot Q}{2 \pi R L_{\rm f}}
\ee
...
\be
F_{12} = \sqrt{F_{\rm 12, H}^2 + F_{\rm 12, V}^2}
\label{eq:solid_flame_F12_HV}
\ee
where $F_{\rm 12, H}$ and $F_{\rm 12, V}$ are given by
\begin{align}
F_{\rm 12, H} &= \frac{\left( B - \frac{1}{S} \right)}{\pi \sqrt{B^2 - 1}} \tan^{-1} \sqrt{ \frac{(B+1)(S-1)}{(B-1)(S+1)} } -
\frac{\left( A - \frac{1}{S} \right)}{\pi \sqrt{A^2 - 1}} \tan^{-1} \sqrt{ \frac{(A+1)(S-1)}{(A-1)(S+1)} } \\
F_{\rm 12, V} &= \frac{1}{\pi S} \tan^{-1} \left( \frac{H}{\sqrt{S^2-1}} \right) - \frac{H}{\pi S} \tan^{-1} \sqrt{\frac{S-1}{S+1}} +
...
flux target~(\si{m}), and $L_{\rm f}$ is the flame height given in Eq.~\ref{eq:point_source_Lf}~(\si{m}).
...
\be
F_{12} = F_{\rm 12, V1} + F_{\rm 12, V2}
\label{eq:solid_flame_F12_V1V2}
\ee
where $F_{\rm 12, V1}$ and $F_{\rm 12, V2}$ are given by
\begin{align}
F_{\rm 12, V1} &= \frac{1}{\pi S} \tan^{-1} \left( \frac{H_1}{\sqrt{S^2-1}} \right) - \frac{H_1}{\pi S} \tan^{-1} \sqrt{\frac{S-1}{S+1}} +
\frac{A_1 H_1}{\pi S \sqrt{A_1^2-1}} \tan^{-1} \sqrt{\frac{(A_1+1)(S-1)}{(A_1-1)(S+1)}} \\
F_{\rm 12, V2} &= \frac{1}{\pi S} \tan^{-1} \left( \frac{H_2}{\sqrt{S^2-1}} \right) - \frac{H_2}{\pi S} \tan^{-1} \sqrt{\frac{S-1}{S+1}} +
...
and $L_{\rm f}$ is the flame height given in Eq.~\ref{eq:point_source_Lf}~(\si{m}).
