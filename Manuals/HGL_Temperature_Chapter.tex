...
For a compartment with natural ventilation, the correlation of McCaffrey, Quintiere, and Harkleroad (MQH)~\cite{SFPE:Walton} predicts that the hot gas layer (HGL) temperature rise, $\Delta T_{\rm g}$~(\si{\celsius}), is given by
\be
\Delta T_{\rm g} = 6.85 \left( \frac{\dot Q^2}{A_{\rm o} \sqrt{H_{\rm o}} h_{\rm k} A_{\rm T}} \right)^{1/3}
\label{eq:MQH}
\ee
where $\dot Q$ is the total heat release rate (HRR) of the fire~(\si{kW}), $A_{\rm o}$ is the area of the ventilation opening~(\si{m^2}), $H_{\rm o}$ is the height of the ventilation opening~(\si{m}), and $A_{\rm T}$ is the total area of the compartment enclosing surfaces~(\si{m^2}), excluding areas of vent openings, which is given by
\be
A_T = 2 L W + 2 L H + 2 W H - H_{\rm o} W_{\rm o}
\label{eq:A_T}
\ee
where $L$, $W$, $H$ are the length, width, and height of the compartment~(\si{m}), respectively, and $W_{\rm o}$ is the width of the ventilation opening~(\si{m}). The heat transfer coefficient, $h_{\rm k}$ (\si{kW/(m^2.K})), is given by
\be
h_{\rm k} = \left\{ \begin{array}{cl}
   \sqrt{k \rho c/t}  & t \le t_{\rm p} \\[0.1in]
   k/\delta           & t > t_{\rm p}
   \end{array} \right.
\label{eq:MQH_hk_lt}
\ee
where $k$ is the thermal conductivity of the interior lining~(\si{kW/(m.K)}), $\rho$ is its density~(\si{kg/m^3}), $c$ is its specific heat~(\si{kJ/(kg.K)}), and $\delta$ is its thickness~(\si{m}). The thermal penetration time, $t_{\rm p}$ (\si{\second}), is given by
\be
t_{\rm p} = \left( \frac{\rho c}{k} \right) \left( \frac{\delta}{2} \right)^2
\label{eq:MQH_tp}
\ee
...
$W$ (m)                     &  4.0                                             \\ \hline
$H$ (m)                     &  3.0                                             \\ \hline
$H_{\rm o}$ (m)               &  2.06                                            \\ \hline
$W_{\rm o}$ (m)               &  0.76                                            \\ \hline
...
The second parameter, $\widetilde{\sigma}_{\rm M}$, is the relative standard deviation of the model, which indicates the variability of the model. In Fig.~\ref{HGL Temperature; Natural Ventilation (MQH)}, there are two sets of off-diagonal lines. The first set, shown as dashed black lines, indicate the uncertainty of the experimental measurements in terms of a relative standard deviation, $\widetilde{\sigma}_{\rm E}$. The experimental relative standard deviation was determined by considering the systematic and random uncertainty values for each measurement quantity, which is described in more detail in the ``Experimental Uncertainty'' section of the FDS Validation Guide~\cite{FDS_Validation_Guide}. The slopes of the dashed black lines are~$1 \pm 2\widetilde{\sigma}_{\rm E}$, which represents the \SI{95}{\percent} confidence intervals. The set of red dashed lines indicate the model relative standard deviation, $\widetilde{\sigma}_{\rm M}$. The model relative standard deviation is reported as one standard deviation of the predicted quantity. The slopes of these lines are~$\delta \pm 2\widetilde{\sigma}_{\rm M}$. If the model was as accurate as the measurements against which it is compared, then the two sets of off-diagonal lines would merge. The extent to which the data scatters outside of the experimental bounds is an indication of the degree of uncertainty of the empirical correlations.
...
For a compartment with forced ventilation, the correlation of Foote, Pagni, and Alvares (FPA)~\cite{SFPE:Walton} predicts that the HGL temperature rise, $\Delta T_{\rm g}$~(\si{\celsius}), is given by
\be
\Delta T_{\rm g} = \left[ 0.63 \left( \frac{\dot Q}{\dot m_{\rm g} c_{\rm p} T_\infty} \right)^{0.72} \left( \frac{h_{\rm k} A_{\rm T}}{\dot m_{\rm g} c_{\rm p}} \right)^{-0.36} \right] T_\infty
\label{eq:FPA}
\ee
where $\dot Q$ is the HRR of the fire~(\si{kW}), $\dot m_{\rm g}$ is the compartment ventilation mass flow rate~(\si{kg/s}), $c_{\rm p}$ is the specific heat of air (\si{kJ/(kg.K)}), $T_\infty$ is the ambient air temperature~(\si{\celsius}), $h_{\rm k}$ is the heat transfer coefficient~(\si{kW/(m^2.K)}), and $A_{\rm T}$ is the total area of the compartment enclosing surfaces~(\si{m^2}), excluding areas of vent openings. The heat transfer coefficient, $h_{\rm k}$ (\si{kW/(m^2.K)}), is given by
\be
h_{\rm k} = \left\{ \begin{array}{cl}
   \sqrt{k \rho c/t}  & t \le t_{\rm p} \\[0.1in]
   k/\delta           & t > t_{\rm p}
...
For a compartment with forced ventilation, the correlation of Deal and Beyler (DB)~\cite{SFPE:Walton} predicts that the HGL temperature rise, $\Delta T_{\rm g}$~(\si{\celsius}), is given by
\be
\Delta T_{\rm g} = \left( \frac{\dot Q}{\dot m_{\rm g} c_{\rm p} + h_{\rm k} A_{\rm T}} \right)
\label{eq:DB}
\ee
where $\dot Q$ is the HRR of the fire~(\si{kW}), $\dot m_{\rm g}$ is the compartment ventilation mass flow rate~(\si{kg/s}), $c_{\rm p}$ is the specific heat of air (\si{kJ/(kg.K)}), $T_\infty$ is the ambient air temperature~(\si{\celsius}), $h_{\rm k}$ is the heat transfer coefficient~(\si{kW/(m^2.K)}), and $A_{\rm T}$ is the total area of compartment enclosing surfaces~(\si{m^2}), excluding areas of vent openings. The heat transfer coefficient, $h_{\rm k}$ (\si{kW/(m^2.K)}), is given by
\be
h_{\rm k} = 0.4\ \mathrm{max} \left( \sqrt{\frac{k \rho c}{t}} , \frac{k}{\delta} \right)
\label{eq:DB_hk}
\ee
...
For a compartment with no ventilation (closed doors) and constant HRR, the correlation of Beyler~\cite{SFPE:Walton} predicts that the HGL temperature rise, $\Delta T_{\rm g}$~(\si{\celsius}), is given by
\be
\Delta T_{\rm g} = \frac{2 K_2}{K_1^2} (K_1 \sqrt{t} - 1 + e^{-K_1 \sqrt{t}})
\label{eq:Beyler}
\ee
where $t$ is the exposure time~(\si{\second}). $K_1$ is given by
\be
K_1 = \frac{2(0.4\sqrt{k \rho c}) A_{\rm T}}{m c_{\rm p}}
\label{eq:Beyler_K1}
\ee
where $k$ is the thermal conductivity of the interior lining~(\si{kW/(m.K)}), $\rho$ is the density of the interior lining~(\si{kg/m^3}), $c$ is the specific heat of the interior lining~(\si{kJ/(kg.K)}), $A_{\rm T}$ the total area of compartment enclosing surfaces~(\si{m^2}), $m$ is the mass of gas in the compartment~(\si{kg}), and $c_{\rm p}$ is the specific heat of air~(\si{kJ/(kg.K)}). $K_2$ is given by
\be
K_2 = \frac{\dot Q}{m c_{\rm p}}
\label{eq:Beyler_K2}
\ee
