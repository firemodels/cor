...
Note that some of these cases assume a quasi-steady approach for a fire source $\dot Q$ that follows a specified $t$-squared growth rate, which was specified as $\dot Q = \alpha t^2$ up to a cutoff time of $t_{\rm fire}$. In this approach, for a given time and HRR, Eq.~\ref{eq:Alpert} was used to calculate the time that the detector would activate. If the calculated activation time was less than the current time, then the detector was assumed to activate. After the time $t_{\rm fire}$, the fire HRR was steady.
...
\multicolumn{2}{|l|}{$\Delta T_{\rm c}$ (\si{\celsius})}  &  \multicolumn{1}{c|}{5}           \\ \hline
\multicolumn{2}{|l|}{$r$ (m)}                           &  \multicolumn{1}{c|}{1.3}         \\ \hline
\multicolumn{2}{|l|}{$H$ (m)}                           &  \multicolumn{1}{c|}{2.1}         \\ \hline
\multicolumn{2}{|l|}{$t_{\rm fire}$ (s)}                  &  \multicolumn{1}{c|}{300}         \\ \hline
