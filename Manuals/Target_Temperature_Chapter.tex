...
The net heat flux at the surface of the cable is determined from the exposing gas temperature surrounding the cable at the $n$-th time step, $T_{\rm g}(t^n)$
\be
\dot q''(t^n) = \varepsilon \sigma (T_{\rm g}(t^n)^4 - (T_{\rm s}^n)^4) + h (T_{\rm g}(t^n) - T_{\rm s}^n)
\label{eq:cable_net_heat_flux}
\ee
where $\varepsilon$ is the emissivity of the cable surface (assumed to be 0.95), $\sigma$ is the Stefan-Boltzmann constant (\SI{5.67e-8}{W/(m^2.K^4)}), $h$ is the convective heat transfer coefficient (assumed to be \SI{10}{W/(m^2.K)}, which is typical of free convection), $T_{\rm g}(t^n)$ is the effective gas temperature at the $n$-th time step~(\si{K}), and $T_{\rm s}^n$ is the surface temperature of the cable~(\si{K}).
...
A conduit forms a thermal barrier between the hot gases of a fire and the cable itself. A simple way to incorporate its effect into the THIEF model is to replace the ``exposing'' gas temperature, $T_{\rm g}$, in Eq.~\ref{eq:cable_net_heat_flux} by the conduit's temperature, $T_{\rm c}$. In other words, the cable no longer ``sees'' the hot gases from the fire, but rather the interior surface of the conduit.
...
\be
\dot q''_{\rm ext}(t^n) = \varepsilon_{\rm c} \sigma (T_{\rm g}(t^n)^4 - (T_{\rm c}^n)^4) + h (T_{\rm g}(t^n) - T_{\rm c}^n)
\ee
where $T_{\rm c}^n$ is the conduit temperature at the $n$-th time step~(\si{K}), and $\varepsilon_{\rm c}$ is the emissivity of its surface (assumed to be 0.85, which is typical of non-polished steel). Heat is transferred from the interior surface of the conduit to the cable surface via radiation and convection
\be
\dot q''_{\rm int}(t^n) = F \sigma ((T_{\rm c}^n)^4 - (T_{\rm s}^n)^4) + h (T_{\rm c}^n - T_{\rm s}^n)
\ee
where
\be
F = \left( \left( \frac{R_{\rm c}}{R} \right) \frac{1}{\varepsilon} + \frac{1-\varepsilon_{\rm c}}{\varepsilon_{\rm c}} \right)^{-1}
\ee
where $R_{\rm c}$ is the inner radius of the conduit~(\si{m}). The view factor, $F$, was based on the assumption that the conduit and cable are concentric cylinders~\cite{CAROLFIRE}.
...
The temperature rise, $\Delta T_{\rm s}$~(\si{\celsius}), of an unprotected steel member exposed to fire can be predicted using~\cite{SFPE:Milke2}
\be
\Delta T_{\rm s} = \frac{F}{V} \frac{1}{\rho_{\rm s} c_{\rm s}} \left[ h_{\rm c} (T_{\rm f} - T_{\rm s}) + \sigma \epsilon (T_{\rm f}^4 - T_{\rm s}^4) \right] \Delta t
\label{eq:unprotected_steel}
\ee
where $F/V$ is the ratio of heated surface area to volume~(\si{m^{-1}}), $\rho_{\rm s}$ is the density of steel~(\si{kg/m^3}), $c_{\rm s}$ is the specific heat of steel~(\si{J/(kg.K)}), $h_{\rm c}$ is the convective heat transfer coefficient~(\si{W/(m^2.K)}), $T_{\rm f}$ is the exposing fire temperature~(\si{K}), $T_{\rm s}$ is the steel temperature~(\si{K}), $\sigma$ is the Stefan-Boltzmann constant (\si{W/(m^2.K^4)}), $\epsilon$ is the flame emissivity, and $\Delta t$ is the time step~(\si{s}). Note that the HGL temperature, plume temperature, or other exposing temperature can be used as the fire temperature, $T_{\rm f}$.
...
Note: In this verification case, the McCaffrey method (see Section~\ref{sec:McCaffrey}) is used to calculate the exposing fire temperature, $T_{\rm f}$.
...
\multicolumn{2}{|l|}{$\rho_{\rm s}$ (kg/m$^3$)}     &  \multicolumn{1}{c|}{7833}        \\ \hline
\multicolumn{2}{|l|}{$c_{\rm s}$ (\si{kJ/(kg.K)})}  &  \multicolumn{1}{c|}{0}           \\ \hline
\multicolumn{2}{|l|}{$\epsilon$}                  &  \multicolumn{1}{c|}{0}           \\ \hline
\multicolumn{2}{|l|}{$h_{\rm c}$ (\si{W/(m^2.K)})}  &  \multicolumn{1}{c|}{25}          \\ \hline \hline
\multicolumn{2}{|l|}{Correlation for $T_{\rm f}$}   &  \multicolumn{1}{c|}{McCaffrey}   \\ \hline \hline
...
The temperature rise, $\Delta T_{\rm s}$~(\si{\celsius}), of a protected steel member exposed to fire can be predicted, but we must first determine if the thermal capacity of the insulation layer should be accounted for or if it can be neglected~\cite{SFPE:Milke2}.
\be
\Delta T_{\rm s} = \left\{ \begin{array}{cl}
   k_{\rm i} \left( \frac{T_{\rm f} - T_{\rm s}}{c_{\rm s} h \frac{W}{D}} \right) \Delta t        &  c_{\rm s} \frac{W}{D} > 2 c_{\rm i} \rho_{\rm i} h \\[0.1in]
   \frac{k_{\rm i}}{h} \left( \frac{T_{\rm f} - T_{\rm s}}{c_{\rm s} \frac{W}{D} + \frac{1}{2} c_{\rm i} \rho_{\rm i} h} \right) \Delta t  &  c_{\rm s} \frac{W}{D} < 2 c_{\rm i} \rho_{\rm i} h
   \end{array} \right.
\label{eq:protected_steel}
\ee
where $k_{\rm i}$ is the thermal conductivity of the insulation material~(\si{W/(m.K)}), $T_{\rm f}$ is the exposing fire temperature~(\si{K}), $T_{\rm s}$ is the steel temperature~(\si{K}), $c_{\rm s}$ is the specific heat of steel~(\si{J/(kg.K)}), $c_{\rm i}$ is the specific heat of the insulation material~(\si{J/(kg.K)}), $h$ is the thickness of the insulation~(\si{m}), $W/D$ is the ratio of the weight of steel section per unit length to the heated perimeter~(\si{kg/m^2}), $\rho_{\rm i}$ is the density of the insulating material~(\si{kg/m^3}), and $\Delta t$ is the time step~(\si{s}). Note that the HGL temperature, plume temperature, or other exposing temperature can be used as the fire temperature, $T_{\rm f}$.
...
Note: In this verification case, the MQH method (see Section~\ref{sec:MQH}) is used to calculate the exposing fire temperature, $T_{\rm f}$.
...
\multicolumn{2}{|c|}{}                            &  \multicolumn{1}{c|}{}            \\
\multicolumn{2}{|l|}{\rb{Parameter}}              &  \multicolumn{1}{c|}{\rb{Value}}  \\ \hline \hline
\multicolumn{2}{|l|}{$c_{\rm s}$ (\si{kJ/(kg.K)})}  &  \multicolumn{1}{c|}{0.450}       \\ \hline
\multicolumn{2}{|l|}{$W/D$ (kg/m$^2$)}            &  \multicolumn{1}{c|}{50.1}        \\ \hline
\multicolumn{2}{|l|}{$k_{\rm i}$ (\si{W/(m.K)})}    &  \multicolumn{1}{c|}{0.10}        \\ \hline
\multicolumn{2}{|l|}{$\rho_{\rm i}$ (kg/m$^3$)}     &  \multicolumn{1}{c|}{208}         \\ \hline
\multicolumn{2}{|l|}{$c_{\rm i}$ (\si{kJ/(kg.K)})}  &  \multicolumn{1}{c|}{2.0}         \\ \hline
\multicolumn{2}{|l|}{$h_{\rm i}$ (m)}               &  \multicolumn{1}{c|}{0.0191}      \\ \hline \hline
\multicolumn{2}{|l|}{Correlation for $T_{\rm f}$}   &  \multicolumn{1}{c|}{MQH}         \\ \hline \hline
...
\multicolumn{2}{|l|}{$H_{\rm o}$ (m)}               &  \multicolumn{1}{c|}{2.82}        \\ \hline
\multicolumn{2}{|l|}{$W_{\rm o}$ (m)}               &  \multicolumn{1}{c|}{2.4}         \\ \hline
